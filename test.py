...
from main import get_response, validate_spotify_url, ValidationError, build_track_url, get_url, get_meta_props, \
    get_song_data, NoDataError, lambda_handler, handle_error
from test_data import API_GATEWAY_EVENT, SPOTIFY_TRACK_HTML_CONTENT, EXPECTED_META_PROPS, \
    SPOTIFY_TRACK_HTML_CONTENT_WITHOUT_META


def make_api_gateway_event(url) -> Dict[str, Any]:
...
class TestValidateSpotifyUrl(unittest.TestCase):
    ...
    def test_invalid(self):
        ...
        invalid_identifiers = [
            ...
            f"https://open.spotify.com/track?si=76e0e46d5d7742e0",
            f"track:{track_id}",
            track_id + "/"
        ]
...
class TestGetMeta(unittest.TestCase):
    ...
    def test_valid_html_without_meta(self):
...
class TestGetResponse(unittest.TestCase):
    ...
    def test_get_response_successfully(self):
    ...
    def test_get_response_no_data_in_content(self):
        request = make_api_gateway_event("https://open.spotify.com/track/1UUBkjBgce8P53hQC3nHtU")
...
class TestLambdaHandler(unittest.TestCase):
    ...
    def test_success(self, mocked_get):
        ...
        response._content = SPOTIFY_TRACK_HTML_CONTENT.encode("utf-8")
    ...
    def test_error(self):
        ...
        for error in errors:
            with self.subTest(error=error.__class__.__name__):
                with mock.patch("main.get_response", side_effect=lambda *args, **kwargs: raise_error(error)):
    ...
    def test_no_url_error(self):
        event = make_api_gateway_event(None)
        response = lambda_handler(event, {})
        self.assertIsInstance(response, dict)
        self.assertIn("statusCode", response)
        self.assertIn("body", response)
